    ...
    if showMap:
        for row in range(MAP_SIZE):
            for col in range(MAP_SIZE):
                ...
                if MAP[square] == "^" : color = (200, 0, 102)
                elif MAP[square] == "+" : color = (50, 50, 50)
                elif MAP[square] == "#" : color = (0, 0, 102)
                else : color = (0, 0, 0)
        ...
        int(player_x) + int(SCREEN_WIDTH - (MAP_SIZE * TILE_SIZE)), int(player_y) + SCREEN_HEIGHT - int(MAP_SIZE * TILE_SIZE)),
                           6)  # PLAYER
    ...
    for ray in range(CASTED_RAYS):
        for dephh in range(int(MAX_DEPTH / uncalc)):
            ...
            if MAP[cube] == '#':
                ...
                pygame.draw.rect(win, (color, color, color), (SCREEN_HEIGHT + ray * SCALE - (SCREEN_WIDTH / 2) + screenPatch, (SCREEN_HEIGHT / 2) - wall_height / 2, SCALE, wall_height),)
...
while True:
    ...
    xTarget = (MAP.index("+")%30+0.5)*25
    yTarget = (MAP.index("+")//30+0.5)*25

    if player_x+divideSpeedMax < xTarget : player_x+=divideSpeedMax
    elif player_x-divideSpeedMax > xTarget : player_x -= divideSpeedMax
    elif player_y+divideSpeedMax < yTarget : player_y += divideSpeedMax
    elif player_y-divideSpeedMax > yTarget : player_y -= divideSpeedMax



    if not keys[pygame.K_DOWN] and not keys[pygame.K_LEFT] and not keys[pygame.K_UP] and not keys[pygame.K_RIGHT]: forward = False
